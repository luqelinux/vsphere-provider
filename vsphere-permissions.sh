...
govc permissions.set -propagate=false -principal $CAPI_USER -role ReadOnly /$DATACENTER/host/$CLUSTER
...
govc permissions.set -propagate=false -principal $CAPI_USER -role CAPI-VCENTER /
...
govc permissions.set -propagate=false -principal $CAPI_USER -role CAPI-DATASTORE /$DATACENTER/datastore/$DATASTORE

govc role.create CAPI-PORTGROUP Network.Assign
govc permissions.set -propagate=false -principal $CAPI_USER -role CAPI-PORTGROUP /$DATACENTER/network/$NETWORK
